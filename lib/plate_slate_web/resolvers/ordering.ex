defmodule PlateSlateWeb.Resolvers.Ordering do
  ...
  def ready_order(_, %{id: id}, _) do
    ...
    with {:ok, order} <- Ordering.update_order(order, %{state: "ready"}) do
      {:ok, %{order: order}} |> IO.inspect(label: :ok)
    else
      {:error, changeset} ->
      {:ok, %{errors: transform_errors(changeset)}} |> IO.inspect(label: :error)
    end
  end

  def complete_order(_, %{id: id}, _) do order = Ordering.get_order!(id)
